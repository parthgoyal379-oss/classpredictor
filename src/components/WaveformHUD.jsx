...
export default function WaveformHUD() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      ctx.beginPath();
      ctx.lineWidth = 1.8;
      ctx.strokeStyle = "rgba(0, 223, 216, 0.85)";
      for (let x = 0; x < width; x++) {
        const y = height / 2 + Math.sin(x * 0.035 + step) * 16 * Math.sin(x * 0.015);
      ...
      for (let x = 0; x < width; x++) {
        const y = height / 2 + Math.cos(x * 0.025 - step * 1.2) * 12 * Math.cos(x * 0.02);
        if (x === 0) ctx.moveTo(x, y);
        else ctx.lineTo(x, y);
      }
      ctx.stroke();
  ...
  return (
    ...
    >
      ...
      <div style={{ height: "46px", width: "100%", position: "relative", marginBottom: "0.75rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "0.5rem", textAlign: "center" }}>
        {[
          ...
          <div key={item.label} style={{ background: "rgba(255, 255, 255, 0.03)", borderRadius: 6, padding: "0.35rem" }}>
